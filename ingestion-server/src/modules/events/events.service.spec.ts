    ...
    describe('CreateInteractionEvent', () => {
        ...
        it('Creates interation events matching the input', async () => {
            ...
            for(let i = 0; i < results.length; i++) {
                expect(results[i].projectId).toEqual(validEvents[i].projectId);
                expect(results[i].userId).toEqual(validEvents[i].userId);
                expect(results[i].itemId).toEqual(validEvents[i].itemId);
                expect(results[i].eventType).toEqual(validEvents[i].eventType);
                expect(results[i].interactionValue).toEqual(validEvents[i].interactionValue);
            }
        });
    });
})